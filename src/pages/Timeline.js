...
import ButtonNewPosts from "../components/ButtonNewPosts";
...
export default function Timeline() {
  ...
  const URL = "https://projeto17-linkr-back-end.herokuapp.com/posts";
    ...
    useInterval(() => {
      console.log("executando mais 15 segundos");
      (async () => {
        try {
          axios.get("https://projeto17-linkr-back-end.herokuapp.com/posts", {
              headers: { Authorization: `Bearer ${token}` }
            ...
            .then((response) => {
              ...
              if(response.data.length > posts.length) {
                // console.log("tem post a mais")
                // console.log(response.data.slice(- (response.data.length - posts.length)))
                setNewPosts(response.data.slice( - (response.data.length - posts.length)))
        ...
        <Container>
          <Posts>
            <AllPosts>
              ...
              >
                {newposts > 0 ? <ButtonNewPosts qntNewPosts={newposts.length} onClick={() => setPosts([...posts, newposts])}/> : <></>}
                {posts ? (
                  posts.length > 0 ? (
